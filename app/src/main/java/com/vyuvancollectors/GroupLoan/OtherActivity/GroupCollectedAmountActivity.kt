...
import com.vyuvancollectors.databinding.ActivityGroupCollectedAmountBinding
...
class GroupCollectedAmountActivity : AppCompatActivity() {

    private var binding : ActivityGroupCollectedAmountBinding? = null
    private var recyclerView : GroupCollectedAmountRv? = null
...
//        json.addProperty("collectionType","$month")
        ...
        val call = apiClient?.postTwoHeadersWithTokenData(
            ...
            "v1/emi/paidEmiList",jsonObjectInBody
        )
        call?.enqueue(object : Callback<JsonObject> {
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                if (response.isSuccessful) {
                    val list = ArrayList<GroupCollectedAmountData>()
                    ...
                    if (status == true) {
                        for (i in 0 until jsonArrayMain.length()) {
                            ...
                            val lastDateOfCollect = jsonArrayMain.getJSONObject(i).getString("lastDateCollected")
                        ...
                        binding?.overdueEmiRv?.layoutManager =
                            LinearLayoutManager(this@GroupCollectedAmountActivity)
                        recyclerView = GroupCollectedAmountRv(list)
