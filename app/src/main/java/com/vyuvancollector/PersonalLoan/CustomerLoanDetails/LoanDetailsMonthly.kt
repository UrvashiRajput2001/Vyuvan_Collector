package com.vyuvancollector.PersonalLoan.CustomerLoanDetails
...
import com.vyuvancollector.PersonalLoan.EMIDetailsAdapter.EmiListMonthlyAdapter
import com.vyuvancollector.PersonalLoan.Emi_Data_Class.MonthlyEmiModal
import com.vyuvancollector.PersonalLoan.PDF.Custom.DataForYearlyPdf
import com.vyuvancollector.PersonalLoan.PDF.Custom.PdfYearlyEmiData
import com.vyuvancollector.PersonalLoan.PDF.Monthly.DataForMonthlyPdf
import com.vyuvancollector.PersonalLoan.PDF.Monthly.MonthlyPdfConverter
...
import com.vyuvancollector.databinding.ActivityLoanDetailsMonthlyBinding
import com.google.gson.JsonObject
import com.vyuvancollector.PersonalLoan.PDF.Daily.DailyPdfConverter
import com.vyuvancollector.PersonalLoan.PDF.Daily.DataForDailyPdf
import com.vyuvancollector.PersonalLoan.PDF.Daily.PdfDailyEmiData
...
class LoanDetailsMonthly : AppCompatActivity() {

    private var binding : ActivityLoanDetailsMonthlyBinding? = null
    private var recyclerView : EmiListMonthlyAdapter? = null
    private var PERMISSION_CODE = 101

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityLoanDetailsMonthlyBinding.inflate(layoutInflater)
...
//            Toast.makeText(applicationContext, "Internet Connected", Toast.LENGTH_SHORT).show()
    ...
    private fun monthlyEMIListAPI(){
        ...
        val call = apiClient?.postDataGET2(token,"v1/emi/$agentId/$customerId")
        call?.enqueue(object  : Callback<JsonObject> {
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                if (response.isSuccessful) {
                    var list = ArrayList<MonthlyEmiModal>()
                    ...
                    if (status == true) {
                        for (i in 0 until jsonArray.length()) {
                            ...
                            binding?.monthlyEmiRv1?.layoutManager = LinearLayoutManager(this@LoanDetailsMonthly)
                            recyclerView = EmiListMonthlyAdapter(list)
                            binding?.monthlyEmiRv1?.adapter = recyclerView
                        }
                    }
                }
    ...
    private fun forLoanDetails(){
        ...
        val apiClient = ApiClient.getInstance()?.create(ApiInterface::class.java)
        val call = apiClient?.postDataGET2(token,"v1/loans/getCustomerLoanDetails/$agentId/$mobile")
        call?.enqueue(object  : Callback<JsonObject> {
    ...
    private fun forPdfLoanDetailsApi(){
        ...
        val apiClient = ApiClient.getInstance()?.create(ApiInterface::class.java)
        val call = apiClient?.postDataGET2(token,"v1/emi/getLoanEmis/$agentId/$customerId1")
        call?.enqueue(object : Callback<JsonObject>{
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                    ...
                    if (status == "true") {
                        for (i in 0 until jsonArray.length()){
                            ...
                            for (j in 0 until jsonArray2.length()){
                                ...
                                emilist.add(PdfDailyEmiData(remainingAmount, emiAmount, dateOfCollect))
                            }
                            list.add(
                                DataForDailyPdf(
                                    name,
