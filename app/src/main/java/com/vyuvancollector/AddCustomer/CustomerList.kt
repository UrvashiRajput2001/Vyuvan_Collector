...
import com.vyuvancollector.databinding.ActivityCustomerListBinding
...
class CustomerList : AppCompatActivity() {
    ...
    private fun customerListApi(){
        ...
        val call = apiClient?.postDataGET2(token.toString(),"v1/customers/getAllCustomer/$agentId")
                ...
                if (response.isSuccessful) {
                    ...
                    if (status == true) {
                        for (i in 0 until jsonArray.length()) {
                            ...
                            list.add(CustomerDetialsData(name,email,phone,whatsapp,address))
    ...
    private fun forSearch() {
        ...
        val apiClient = ApiClient.getInstance()?.create(ApiInterface::class.java)
        val call = apiClient?.postDataGET2(token.toString(),"v1/customers/getCustomerByPhone/$agentId/$phone")
        call?.enqueue(object  : Callback<JsonObject> {
            @SuppressLint("NotifyDataSetChanged")
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                if (response.isSuccessful) {
